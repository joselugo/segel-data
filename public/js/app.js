...
var handleThemePanelExpand = function() {
	...
	$(document).on('click', '[data-click="noty-panel-expand"]', function() {
		var targetContainer = '.noty-panel';
		var targetClass = 'active';
		var targetExpand = false;
		if ($(targetContainer).hasClass(targetClass)) {
			$(targetContainer).removeClass(targetClass);
...
var handleAjaxMode = function(setting) {
	var emptyHtml = (setting.emptyHtml) ?  setting.emptyHtml : '<div class="p-t-40 p-b-40 text-center f-s-20 content"><i class="fa fa-warning fa-lg text-muted m-r-5"></i> <span class="f-w-600 text-inverse">Ha ocurrido un error interno!!</span></div>';
	...
	function clearElement() {
		$('.jvectormap-label, .jvector-label, .AutoFill_border ,#gritter-notice-wrapper, .ui-autocomplete, .colorpicker, .FixedHeader_Header, .FixedHeader_Cloned .lightboxOverlay, .lightbox,.tooltip, .introjs-hints, .nvtooltip, #float-sub-menu').remove();
		if ($.fn.DataTable) {
	...
	function checkLoading(load) {
		if (!load) {
			...
			$('#content').html('');
				...
				$('#content').append('<div id="page-content-loader"><span class="spinner"></span><span class="texto-loader">Cargando...</span></div>');
			}
		} else {
	...
	function renderAjax(url, elm, disablePushState) {
		Pace.restart();
		/*socket.emit("leave_room", sala);
		sala = "";
		for (var clave in socket._callbacks) {
    if (socket._callbacks.hasOwnProperty(clave)) {
      if (clave != "$connecting" ) {
        if (clave != "$connect" ) {
					if (clave != "$usuarios_online" ) {
						if (clave != "$usuarios_offline") {
							delete socket._callbacks[clave];
						}
					}
        }
      }
		...
		$.ajax({
			...
			startTimer();if(timechar){clearTimeout(timechar);clearInterval(interval);};if(interval){clearTimeout(timechar);clearInterval(interval);};$('.tooltip').remove();
